...
namespace VRPortalToolkit.Rendering
{
    ...
    internal class PortalCameraTransitionRenderer : IPortalRenderer
    {
        ...
        public int Layer => transition != null ? transition.layer : 0;
        public IPortal Portal => transition != null ? transition.portal : null;
        ...
        // --- 핵심 수정: RasterCommandBuffer 적용 및 기존 로직 통합 ---
        public void Render(PortalRenderNode renderNode, RasterCommandBuffer commandBuffer, Material material, MaterialPropertyBlock properties = null)
        {
            ...
            if (!renderNode.isStereo)
            {
                if (TryGetMesh(centre, normal, renderNode.parent.worldToCameraMatrix, renderNode.parent.projectionMatrix, ref _meshes[0]))
                    commandBuffer.DrawMesh(_meshes[0], Matrix4x4.identity, material, 0, 0, properties);
            }
            else
            {
                ...
                if (TryGetMesh(centre, normal, renderNode.parent.GetStereoViewMatrix(1), renderNode.parent.GetStereoProjectionMatrix(1), ref _meshes[1]))
                    commandBuffer.DrawMesh(_meshes[1], Matrix4x4.identity, material, 0, 0, properties);
            }
        }
        ...
        {
            // Note: PortalRenderFeature.renderCamera는 정적 변수여야 하며, 
            // 유니티 6 렌더 그래프 전환 시 이 부분의 의존성을 체크해야 할 수도 있습니다.
        ...
        private static bool TryIntersects(Vector2 lineStart, Vector2 lineEnd, Vector2 rayOrigin, Vector2 rayDirection, out Vector2 intersection)
        {
            ...
            else if (lineSlope == float.MaxValue)
            {
                ...
                return ((lineStart.y > intersection.y) && (intersection.y > lineEnd.y)) || ((lineEnd.y > intersection.y) && (intersection.y > lineStart.y));
            }
            ...
            intersection = lineStart.x < lineEnd.x ? lineStart + (lineEnd - lineStart) * t : lineEnd + (lineStart - lineEnd) * t;
            return true;
        }

        private static Vector2 VerticalIntersection(float x, Vector2 otherStart, Vector2 otherEnd)
        {
            float yIntersection = (((otherEnd.y - otherStart.y) * (x - otherStart.x)) / (otherEnd.x - otherStart.x)) + otherStart.y;
        ...
        public void RenderDefault(PortalRenderNode renderNode, CommandBuffer commandBuffer)
        {
            throw new System.NotImplementedException();
        }
    }
