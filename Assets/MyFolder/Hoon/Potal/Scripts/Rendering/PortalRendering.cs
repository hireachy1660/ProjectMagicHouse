...
using UnityEngine.Rendering.RenderGraphModule; // 유니티 6를 위해 필수 추가
...
namespace VRPortalToolkit.Rendering
{
    ...
    /// <summary>
    /// 유니티 6 렌더 그래프 대응을 위해 RasterCommandBuffer를 사용하도록 수정된 인터페이스입니다.
    /// </summary>
    public interface IPortalRenderer
    {
        ...
        // --- 유니티 6 대응: RasterCommandBuffer 버전으로 단일화 ---

        /// <summary>
        /// 포탈을 지정된 재질로 렌더링합니다. (Unity 6 전용)
        /// </summary>
    ...
    public static class PortalRendering
    {
        ...
        public static void RegisterPortalRenderer(IPortalRenderer renderer) => _allRenderers.Add(renderer);
        public static void UnregisterPortalRenderer(IPortalRenderer renderer) => _allRenderers.Remove(renderer);
        ...
        public static void UnregisterCameraTranstion(Camera camera, IPortalCameraTransition transition)
        {
            ...
            if (index != -1)
            {
                ...
                {
                    renderer = _transitionRenderers.Find((i) => i.camera == camera);
                    if (renderer != null) _transitionByCamera[camera] = renderer;
                    else _transitionByCamera.Remove(camera);
                }
            }
        }
