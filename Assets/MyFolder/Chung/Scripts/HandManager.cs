...
public class HandManager : MonoBehaviour
{
    ...
    private IEnumerator WaitAndInteractAction()
    {
        ...
        {
            // 잡은 물체의 인터페이스 추출
            if(handInteractor.SelectedInteractable.gameObject.transform.parent.TryGetComponent<IItem>(out currentItem))
            {
                Debug.Log("Current Grab Item Is :" + currentItem + currentItem.Transform.gameObject.name);
            }
            else
            {
                Debug.Log($"{handInteractor.SelectedInteractable} [HandManager] Thise Item Is Not IItem Object");
            }
        }
        else
        {
